     ...
     >
      ...
      <div className='grid grid-cols-1 lg:grid-cols-4 gap-6'>
        <AnimatePresence>
          ...
          className='fixed lg:static top-0 left-0 z-50 lg:z-auto w-72 lg:w-auto h-full lg:h-[75vh] lg:rounded-3xl lg:col-span-1 bg-black/90 lg:bg-black/80 backdrop-blur-xl border border-white/10 shadow-[0_2-px_45px_rgba(0,0,0,0.6)]
          p-5 overflow-y-auto'
          >
            <div className='mb-4 space-y-1 '>
            <button onClick={()=>navigate('/notes')} className='w-full px-3 py-2 rounded-lg text-sm text-gray-200 bg-white/10 hover:bg-white/20 text-start'>
              ➕ New Notes
            </button>

            <hr className='border-white/10 mb-4'></hr>
            ...
            <ul className='space-y-3'>
              {topics.map((t,i)=>(
                <li key={i} onClick={()=>{openNotes(t._id)}} className={`cursor-pointer rounded-xl p-3 border transition-all ${
                selectedNote?._id===t._id
                ...
                :"bg-white/5 border-white/10 hover:bg-white/10"
                 ...
                 <div>
                  {t.classLevel && <span className='px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300'> ClassLevel: {t.classLevel}</span>}
                   ...
                   {t.examType && <span className='px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-300'> {t.examType}</span>}
                 </div>
                 <div className='flex gap-3 mt-2 text-xs text-gray-300'>
                  {t.revisionMode && <span> ⚡️Revision</span>}
       ...
       >
        {loading && <p className='text-center text-gray-500'>
          Loading notes...</p>}
